-- Fix: the previous version's status CASE expression evaluated to `text`
-- and failed to implicitly cast to the bill_status enum
-- ("column status is of type bill_status but expression is of type text").
