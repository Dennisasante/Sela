-- Bills need partial-payment support: a one-off bill that's paid in
-- installments should read as "partially paid" with an outstanding balance,
-- not flip straight to "paid" the moment any payment lands.
...
begin
  ...
  if v_bill.is_recurring then
    ...
    update public.bills
    set status = case when v_total_paid >= v_bill.amount then 'paid' else 'partially_paid' end
    where id = v_bill.id;
  end if;
